...
class Input extends React.PureComponent<InputProps, InputState> {
    ...
    renderClear() {
        ...
        return allowClear && this.state.text && <i styleName="my-input-clear" onClick={ this.onClear }></i>
    }
    renderInput() {
        const { id, placeholder, maxLength, disabled, autoFocus, size = "normal" } = this.props;
        return (
            <div styleName={classNames("my-input-input",{
                [`${size}`]: true
            })}>
                <input
                    {...{ id, placeholder, maxLength, disabled, autoFocus }}
